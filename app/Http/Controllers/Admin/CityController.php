...
class CityController extends BaseWebController
{
        ...
        if ($request->ajax()) {
            return Datatables::of(City::query())
                             ...
                             ->addColumn('province_title', function ($row) {
                                 return Str::limit($row->province->title);
                             })
                             ->filterColumn('title', function ($query, $keyword) {
                                 $query->whereHas('province.translations', function ($query) use ($keyword) {
                                     $query
                                         ->where('key', 'title')
    ...
    public function edit(City $city)
    {
        $selectedProvinces = [
            [
                'uuid'   => $city->province->uuid,
                'value' => $city->province->title,
            ],
        ];
        return view('admin.pages.city.edit', compact('city','selectedProvinces'));
    }
